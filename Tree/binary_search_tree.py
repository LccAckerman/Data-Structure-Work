class Node:  # this code makes the tree that we'll traverse
...
"""
Let's assume that duplicates are overriden by the new node that is to be inserted. 
Other options are to keep a counter of duplicate nodes, or to keep a list of
 duplicates nodes with the same value.
"""
...
class Tree:
    ...
    def insert_with_loop(self, new_value):
        ...
        while True:
            ...
            if comparision == 0:  # override with new node's value
                node.set_value(new_node.get_value())
                break  # override node, and stop looping
            elif comparision == -1:  # go left
    ...
    def delete(self, node, key):  # return the root of new tree (這個tree 就是以node為root的tree, 對於每個階層就是sub tree)
        if node is None:
            return node
